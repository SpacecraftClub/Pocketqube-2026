...
int DataPersistance::calculateAltitude(double pressure){
    //TODO implement this to be more accurite
    return (1-pow(pressure/1013.25,.190284)*145366.45);
}

void DataPersistance::appendData(File* destination, double* data, int photoNumber){
...
double* DataPersistance::averageData(){
...
void DataPersistance::addData(double* data){
    for(byte col = 0; col < NUM_ITEMS_IN_CSV; col++){
        switch(col){
            case ALTITUTE:
                break;
...
void DataPersistance::addToCSV(int photoNumber){
